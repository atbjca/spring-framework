...
`spring-framework-6.2` is a Git repository at `spring-framework-6.2`. This change releases `6.2.19-nes.patch.1` from one dedicated source commit and records enough evidence for a fresh session to reconcile the component against Git, OpenSpec, and Nexus.

Representative publications:

`cn.bjca.footstone.bpring:bjca-footstone-bpring-core:6.2.19-nes.patch.1`
...
- Publish `6.2.19-nes.patch.1` with no internal `cn.bjca.footstone` SNAPSHOT metadata.
...
- Verify immutable remote assets before creating annotated tag `v6.2.19-nes.patch.1`.
...
Only the manifest owner for `spring-framework-6.2` may write to this repository while the component is being prepared.

### 2. Publish only a complete internal RELEASE dependency graph

Set the project version to `6.2.19-nes.patch.1` and update every internal dependency, parent, imported BOM, and plugin version to its approved RELEASE. Generate the complete local publication set and scan all generated POM/BOM metadata, not only the representative GAVs.
...
Run representative RELEASE-only consumer validation after local publication. Update `README.md`, `doc/QUICK_START.md`, `doc/GAV_MAPPING.md` before creating the dedicated release commit. Build and deploy from that recorded commit without subsequent source changes.

For this metadata-only RELEASE, the coordinator-approved fast path may reuse archived build and full-test evidence only when the evidence commits are ancestors of HEAD, no application or test source changed afterward, intervening build changes preserve task semantics, and the working diff contains only release metadata, documentation, and OpenSpec evidence. Fresh `make install`, complete generated-POM scanning, and offline local consumer validation remain mandatory.
...
After the one authorized deploy, download representative POM and binary assets from Nexus RELEASE, verify version and metadata, calculate checksums, and run the RELEASE-only consumer smoke test. Do not create or push `v6.2.19-nes.patch.1` until the component reaches `nexus-verified`.
...
Create annotated tag `v6.2.19-nes.patch.1` on the exact release commit used for deployment. Verify the remote branch and tag after push.
