...
- [x] 1.1 Confirm this change is apply-ready and assigned exclusively to `spring-framework-5.3` in `spring-framework-5.3`.
- [x] 1.2 Acquire the component owner lease and record owner/session/timestamps in the release manifest.
- [x] 1.3 Record branch, `origin` fetch/push URLs, HEAD SHA, active OpenSpec changes, toolchain, current version, and target `5.3.39-nes.patch.1` without exposing credentials.
...
- [x] 2.1 Set the component version to `5.3.39-nes.patch.1`.
...
- [x] 2.3 Discover the complete local publication set and reconcile it with the representative GAVs: `cn.bjca.footstone.bpring:bjca-footstone-bpring-core:5.3.39-nes.patch.1`.
...
- [x] 2.5 Update `README.md`, `doc/GAV_MAPPING.md` with version, GAVs, required RELEASE dependencies, validation commands, Nexus location, exclusions, and notable constraints.
- [x] 2.6 Review the full working diff and confirm it contains no unrelated source changes or credentials.

## 3. Local verification

- [x] 3.1 Reuse the explicitly approved, ancestor-bound composite build evidence instead of re-running `make build`; record ancestry and intervening diff.
- [x] 3.2 Reuse the explicitly approved, ancestor-bound composite test evidence instead of re-running `make test`; record ancestry and intervening diff.
...
- [ ] 3.7 Re-run worktree and diff checks, then advance the manifest to `locally-verified` only when all gates pass.
...
- [x] 4.3 Create the dedicated release commit for `spring-framework-5.3` `5.3.39-nes.patch.1`.
...
- [ ] 7.1 Confirm `v5.3.39-nes.patch.1` does not already exist locally or remotely and the target release commit SHA is unchanged.
- [ ] 7.2 Create annotated tag `v5.3.39-nes.patch.1` on the exact release commit only after `nexus-verified`.
