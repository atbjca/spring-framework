...
`spring-framework-5.3` is a Git repository at `spring-framework-5.3`. This change releases `5.3.39-nes.patch.1` from one dedicated source commit and records enough evidence for a fresh session to reconcile the component against Git, OpenSpec, and Nexus.
...
- Publish `5.3.39-nes.patch.1` with no internal `cn.bjca.footstone` SNAPSHOT metadata.
...
- Verify immutable remote assets before creating annotated tag `v5.3.39-nes.patch.1`.
...
Only the manifest owner for `spring-framework-5.3` may write to this repository while the component is being prepared.

### 2. Publish only a complete internal RELEASE dependency graph

Set the project version to `5.3.39-nes.patch.1` and update every internal dependency, parent, imported BOM, and plugin version to its approved RELEASE. Generate the complete local publication set and scan all generated POM/BOM metadata, not only the representative GAVs.
...
Run representative RELEASE-only consumer validation after local publication. Update `README.md`, `doc/GAV_MAPPING.md` before creating the dedicated release commit. Build and deploy from that recorded commit without subsequent source changes.

For this release only, the user explicitly approved reusing development build
and test evidence instead of re-running `make build` and `make test`. Reuse is
valid only because every evidence commit is an ancestor of the starting HEAD,
the intervening diff is recorded, changed production code received targeted
tests plus a later full main compilation, and the release worktree changes no
source or test file. This exception does not waive `make install`, complete
generated-POM scanning, or the local consumer smoke test.
...
After the one authorized deploy, download representative POM and binary assets from Nexus RELEASE, verify version and metadata, calculate checksums, and run the RELEASE-only consumer smoke test. Do not create or push `v5.3.39-nes.patch.1` until the component reaches `nexus-verified`.
...
Create annotated tag `v5.3.39-nes.patch.1` on the exact release commit used for deployment. Verify the remote branch and tag after push.
