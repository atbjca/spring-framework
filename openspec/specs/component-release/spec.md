...
## Purpose
TBD - created by archiving change release-6-2-19-nes-patch-1. Update Purpose after archive.
## Requirements
### Requirement: Exclusive and safe component preparation

The release process MUST assign one active owner to `spring-framework-6.2` and MUST inspect branch, origin, tracked and untracked changes, and active OpenSpec changes before modifying `spring-framework-6.2`.
...
Every generated RELEASE POM or BOM for `6.2.19-nes.patch.1` MUST use approved RELEASE versions for internal `cn.bjca.footstone` dependencies, parents, imported BOMs, and plugins.
...
- **WHEN** `make build-thin`, `make test`, `make install`, the metadata scan, and consumer validation succeed
- **THEN** commands and evidence are recorded and the component may become `locally-verified`

#### Scenario: Approved metadata-only fast path
- **WHEN** archived build and test evidence commits are ancestors of HEAD, no product or test source changed afterward, the release diff is metadata-only, and the coordinator authorizes reuse
- **THEN** those build and test results MAY be reused, but fresh local publication, POM scanning, and offline consumer validation remain mandatory
...
The component SHALL have one dedicated release commit containing `6.2.19-nes.patch.1`, internal RELEASE dependency updates, `README.md`, `doc/QUICK_START.md`, `doc/GAV_MAPPING.md`, approved exclusions, and no unrelated source changes.
...
- **WHEN** no POM, binary, checksum, metadata, or partial module asset exists for `6.2.19-nes.patch.1`
...
After Nexus verification, the coordinator MUST create annotated tag `v6.2.19-nes.patch.1` on the exact release commit used to build and deploy `6.2.19-nes.patch.1`.
