...
## Purpose
TBD - created by archiving change release-5-3-39-nes-patch-1. Update Purpose after archive.
## Requirements
### Requirement: Exclusive and safe component preparation

The release process MUST assign one active owner to `spring-framework-5.3` and MUST inspect branch, origin, tracked and untracked changes, and active OpenSpec changes before modifying `spring-framework-5.3`.
...
Every generated RELEASE POM or BOM for `5.3.39-nes.patch.1` MUST use approved RELEASE versions for internal `cn.bjca.footstone` dependencies, parents, imported BOMs, and plugins.
...
- **WHEN** `make build` and `make test` succeed, or their explicitly approved ancestor-bound development evidence is reconciled against the complete intervening diff, and `make install`, the metadata scan, and consumer validation succeed
- **THEN** commands and evidence are recorded and the component may become `locally-verified`

#### Scenario: Approved development evidence is reused
- **WHEN** the user explicitly waives fresh build and test execution, every evidence commit is an ancestor of the release base, intervening production/test/build changes are reconciled, and the release worktree changes no source or test file
- **THEN** the build and test gates MAY use the recorded composite development evidence, but local publication, generated metadata scanning, and consumer validation MUST still run against the RELEASE version
...
The component SHALL have one dedicated release commit containing `5.3.39-nes.patch.1`, internal RELEASE dependency updates, `README.md`, `doc/GAV_MAPPING.md`, approved exclusions, and no unrelated source changes.
...
- **WHEN** no POM, binary, checksum, metadata, or partial module asset exists for `5.3.39-nes.patch.1`
...
After Nexus verification, the coordinator MUST create annotated tag `v5.3.39-nes.patch.1` on the exact release commit used to build and deploy `5.3.39-nes.patch.1`.
