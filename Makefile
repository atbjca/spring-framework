.PHONY: clean test install deploy build stop help setup-gradle checkstyle format
...
help: ## 显示帮助信息
	@echo ""
	@echo "可用命令:"
	@echo "  make setup-gradle - 用本地精确匹配的 Gradle zip 预热 wrapper 缓存；缺包则回退 HTTPS 下载"
...
# 使用 LOCAL_GRADLE_DIR 下与 Wrapper 配置精确匹配的 Gradle zip 预热缓存
# -----------------------------------------------------------------------------
# setup-gradle：构建前预热 Gradle 发行包（几乎所有 target 的前置依赖）
#
# 为什么这么做：
#   Gradle Wrapper 首次运行会按 gradle-wrapper.properties 里的 distributionUrl
#   从 services.gradle.org 下载发行包。在内网/离线/弱网环境下这一步可能很慢或
#   失败。本 target 调用 scripts/setup-gradle-local.sh，读取 Wrapper 配置所需的
#   精确文件名与 SHA-256，并按官方 HTTPS URL 的缓存命名规则
#   （MD5(url)->base36）复制解压到 ~/.gradle/wrapper/dists/，从而免联网。
#   gradle-wrapper.properties 始终保持可移植的官方 HTTPS URL。
#
# 从哪里找包：
#   默认在 LOCAL_GRADLE_DIR（缺省 ~/dev）中查找 Wrapper 指定的精确 zip。
#   可覆盖：LOCAL_GRADLE_DIR=/path/to/zips make <target>
#
# 会产生什么效果：
#   - 找到精确包且 SHA-256 正确：免网络预热 Wrapper 缓存（幂等，已就绪则跳过）。
#   - 只有其它版本：明确列出但不使用，避免错误版本伪装成已满足 Wrapper。
#   - 找不到精确包：打印提示并以退出码 0 继续，由 Wrapper 按官方 HTTPS URL 下载。
#
# 注意：
...
test: setup-gradle ## 运行测试
	./gradlew test $(TOOLCHAINS)
